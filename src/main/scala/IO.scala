...
object IO {
	...
	private def commonPrefix[S](a: Array[S], b: Array[S]): Int = {
		def common(count: Int): Int = {
			if(count >= a.length || count >= b.length || a(count) != b(count)) count
			else common(count+1)
		}
		common(0)
	}
	...
	private def toPathArray(file: File): Array[String] = {
		def toPathList(f: File, current: List[String]): List[String] = {
			if(f == null) current
			else toPathList(f.getParentFile, f.getName :: current)
		}
	...
	def hash(s: String): String = java.security.MessageDigest.getInstance("SHA").
    digest(s.getBytes).flatMap(hashDigits).mkString
