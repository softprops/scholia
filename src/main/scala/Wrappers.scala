...
private[scholia] object Wrappers
{
...
private[scholia] sealed abstract class Iterable[T] {
	def foreach(f: T => Unit) = toList.foreach(f)
	def toList: List[T]
}
private[scholia] sealed trait Removable[T] {
	def -=(t: T) : Unit
	def --=(all: Iterable[T]) { all.foreach(-=) }
	def --=(all: scala.Iterable[T]) { all.foreach(-=) }
}
private[scholia] sealed trait Addable[T] {
	def +=(t: T) : Unit
	def ++=(all: Iterable[T]) { all.foreach(+=) }
	def ++=(all: scala.Iterable[T]) { all.foreach(+=) }
}
private[scholia] sealed abstract class Set[T] extends Iterable[T] {
	def contains(t: T): Boolean
	def isEmpty: Boolean
}
private[scholia] sealed class SetWrapper[T](val underlying: JSet[T]) extends Set[T]
{
	def isEmpty = underlying.isEmpty
	def contains(t: T) = underlying.contains(t)
	def toList =Wrappers.toList(underlying.iterator)
}
private[scholia] sealed class MutableSetWrapper[T](wrapped: JSet[T]) extends SetWrapper[T](wrapped) with Addable[T] with Removable[T]
{
	def +=(t: T) { underlying.add(t) }
	def -=(t: T) { underlying.remove(t) }
	def readOnly: Set[T] = this
}
private[scholia] final class SortedSetWrapper[T](wrapped: JSortedSet[T]) extends MutableSetWrapper(wrapped)
{
...
private[scholia] sealed abstract class Map[K,V] extends Iterable[(K,V)]
{
	def apply(key: K): V
...
private[scholia] sealed abstract class MapWrapper[K,V](val underlying: JMap[K,V]) extends Map[K,V]
{
...
private[scholia] sealed class MutableMapWrapper[K,V](wrapped: JMap[K,V]) extends MapWrapper[K,V](wrapped) with Removable[K] with Addable[(K,V)]
{
